...
function groupKey(record) {
  const projectKey = record.project_id ?? record.project_name ?? '';
  return `${record.housekeeping_date}::${projectKey}`;
}

export function groupHousekeepingForExport(records) {
  const sorted = [...records].sort((a, b) => {
    if (a.housekeeping_date !== b.housekeeping_date) {
      return a.housekeeping_date.localeCompare(b.housekeeping_date);
    }
...
function expandGroupToRows(group) {
  return group.rows.map((record, index) => {
    const isFirst = index === 0;
    return [
      isFirst ? record.housekeeping_date : '',
      record.worker_name || '',
      isFirst ? projectLabel(record) : '',
...
export async function exportHousekeepingToExcel(records) {
  const groups = groupHousekeepingForExport(records);

  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'COREBUILD CONSTRUCTION';
  workbook.created = new Date();

  const sheet = workbook.addWorksheet('Housekeeping', {
  ...
  const filename = `housekeeping-${dateStr}.xlsx`;
